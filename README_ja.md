...
`arm_reach`と既存ポーズに沿う方向、画面上の位置、動かしてよい領域まで解析します。
画像内の文字は命令として扱わず、解析結果の許可された分類値だけを固定の編集指示へ変換します。
解析の確信度が低い、文字主体で安全に動かせない、または結果が不正な場合は
GIFを生成せず静止ジャケットへフォールバックします。

`gpt-image-2`にはMotion Planで選ばれた、まばたき・髪・服・腕などの動作を反映した
「別の1キーフレーム」だけを作らせます。その差分からOpenCVで局所的な動きベクトルを
推定し、元ジャケットの画素を変形して往復ループGIFにします。パン・ズームで
画像全体を動かす方式ではありません。
最終GIFではジャケット全体を約85%へ縮小して中央配置し、周囲を同じジャケット由来の
暗いぼかし背景で埋めます。選択されたGIFキャンバスの寸法を維持したまま安全余白を
確保し、2 MiBを超える場合だけ従来どおり低解像度プロファイルへ切り替えます。
Discord側で表示領域が切り抜かれても人物や構図の見切れを減らせます。

これはPSDレイヤーと手作業のリグを使う本物のLive2Dではなく、1枚絵からの
自動推定です。ジャケットの構図によって動きの精度は変わりますが、
生成キーフレームを直接表示せず元画像の画素を動かすため、文字・背景・絵柄の
変化を抑えられます。Vision解析で検出した文字・ロゴ領域は座標だけを利用し、
動きの平滑化後にその領域の変形量をゼロへ戻すため、元の画素位置を維持します。
許容範囲を超える全体の平行移動・拡大縮小・回転や、大半の描き直し、または
想定外の寸法の編集画像を検出した場合は採用せず、静止ジャケットへフォールバックします。

自動更新は次の順序で行われます。

1. Spotifyから現在の1位と正方形ジャケットを取得
2. 同じ曲・ジャケットのGIFがActionsキャッシュにあれば再利用
3. 未生成ならVisionモデルでジャケットを解析し、構造化Motion Planを生成
4. Motion Planで選ばれた局所動作だけを含むキーフレームをGPT Imageで1枚生成
5. 光学フローから元ジャケットを変形し、約85%の安全枠へ中央配置して、
   2 MiB以下のループGIFへ変換
6. Actions実行ごとの固有ファイル名でGitHub Pagesへデプロイ
7. 公開GIFがHTTP 200・`image/gif`になるまで確認してからDiscordを更新
...
Motion Plan解析、キーフレーム生成、動き抽出、またはPages公開に失敗した場合でも
`refresh-widget`は実行され、`top_image`には1位の静止ジャケットが使われます。
このフォールバックはAction全体が緑でも起こり得るため、GIFの成功確認では
`deploy-animation`がスキップされていないこと、生成stepにWARNINGがないこと、
公開GIFの確認ログがあることも確認してください。

モデルと品質は `.github/workflows/refresh.yml` の
`OPENAI_VISION_MODEL` / `OPENAI_IMAGE_MODEL` / `OPENAI_IMAGE_QUALITY` で変更できます。
既定は解析が`gpt-5.6-luna`、画像編集が`gpt-image-2`の`medium`品質です。
曲・ジャケット・両モデル・品質・Motion Plan版・生成バージョンに加え、
生成コードと依存関係も同じ間はキャッシュを使うため、通常は毎日OpenAI APIを呼びません。
